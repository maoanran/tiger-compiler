...
typedef enum {
	ARGTYPE_BOOL, ARGTYPE_EMPTY, // expects no argument
	ARGTYPE_INT,
	ARGTYPE_STRING,
} ArgType_t;
...
static void Arg_setHeapSize(int heapSize) {
	Control_heapSize = 1024 * heapSize;
	return;
}
static void Arg_enableGcLog() {
	Control_gcLog = 1;
	return;
}
...
static struct Arg_t allArgs[] = { { "heapSize", "<n>", "set the Java heap size (in kilobytes)", ARGTYPE_INT, Arg_setHeapSize }, { "gcLog", 0, "enable log of GC", ARGTYPE_EMPTY, Arg_enableGcLog } };
...
static void errorNoArg(char *name, char *arg) {
...
static void errorWrongArg(char *name, char *arg, char *input) {
	...
	for (; index < argc;) {
		...
		for (; allArgs[i].action; i++) {
			...
			case ARGTYPE_BOOL: {
				...
				if (index >= argc)
					errorNoArg(allArgs[i].name, allArgs[i].arg);
				...
				if (strcmp(arg, "true") == 0)
					...
					errorWrongArg(allArgs[i].name, allArgs[i].arg, arg);
				allArgs[i].action(b);
				break;
			}
			case ARGTYPE_INT: {
