...
public class ClassTable {
	...
	public void initDecs(String current, java.util.LinkedList<codegen.bytecode.dec.T> decs) {
		ClassBinding cb = this.table.get(current);
		for (codegen.bytecode.dec.T dec : decs) {
			codegen.bytecode.dec.Dec decc = (codegen.bytecode.dec.Dec) dec;
			cb.put(current, decc.type, decc.id);
		}
		this.table.put(current, cb);
	}

	public void initMethod(String current, codegen.bytecode.type.T ret, java.util.LinkedList<codegen.bytecode.dec.T> args, String mid) {
		ClassBinding cb = this.table.get(current);
